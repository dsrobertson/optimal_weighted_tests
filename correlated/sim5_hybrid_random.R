phase_III_sim_5T_random = function(five.T.mean, alpha = 0.05,
                         nsim = 10^4, w.grid2, w.grid3, rho){
  
  m = 5
  
  PoS = w.PoS = h.PoS = matrix(nrow = nsim, ncol = m)
  
  corr.matrix = matrix(rep(rho,5*5), nrow = 5)
  diag(corr.matrix) = 1
  
  for(i in 1:nsim){
    
    mu = c(runif(4, min = 0, 
                 max = five.T.mean), five.T.mean)
    
    #T1 = mu.hat = rnorm(m, mean = mu)
    T1 = mu.hat = as.vector(rmvnorm(1, mean = mu, sigma = corr.matrix))
    
    #T2 = rnorm(m, mean = mu)
    T2 = as.vector(rmvnorm(1, mean = mu, sigma = corr.matrix))
  ...
  return(list(PoS.bonf = PoS.bonf, PoS.w.bonf = PoS.w.bonf,
              dPoS.bonf = dPoS.bonf, dPoS.w.bonf = dPoS.w.bonf, n.NA = n.NA))
